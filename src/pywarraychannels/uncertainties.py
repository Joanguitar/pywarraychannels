...
class UniformTiltPanRoll():
    ...
    def update(self):
        ...
        if self.u_tilt:
            ...
            self.transform = np.array([[tilt_cos, -tilt_sin, 0], [tilt_sin, tilt_cos, 0], [0, 0, 1]])
        else:
            self.transform = np.eye(3)
        if self.u_pan:
            ...
            pan_cos = np.cos(pan)
            pan_sin = np.sin(pan)
            self.transform = np.dot(self.transform, np.array([[pan_cos, 0, -pan_sin], [0, 1, 0], [pan_sin, 0, pan_cos]]), self.transform)
        if self.u_roll:
